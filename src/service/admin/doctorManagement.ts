...
} from "@/validation/doctors.validation";
...
export async function createDoctor(_prevState: any, formData: FormData) {
  try {
    ...
    const response = await Fetcher.post("/user/create-doctor", {
      body: newFormData,
    });
...
export async function getDoctors(queryString?: string) {
  try {
    const response = await Fetcher.get(
      `/doctor/getAllDoctor${queryString ? `?${queryString}` : ""}`
    );
    const result = await response.json();
    return result;
  } catch (error: any) {
...
export async function getDoctorById(id: string) {
  try {
    const response = await Fetcher.get(`/doctor/doctorDetails/${id}`);
    const result = await response.json();
    return result;
  } catch (error: any) {
    console.log(error);
...
export async function updateDoctor(
  ...
  try {
    ...
    const response = await Fetcher.patch(`/doctor/updateDoctor/${id}`, {
      headers: {
...
export async function softDeleteDoctor(id: string) {
  try {
    const response = await Fetcher.delete(`/doctor/soft/${id}`);
    const result = await response.json();
...
export async function deleteDoctor(id: string) {
  try {
    const response = await Fetcher.delete(`/doctor/${id}`);
    const result = await response.json();
